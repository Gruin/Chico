...
exports.run = (bot, message, args) => {
  let user = message.mentions.users.first() || bot.users.cache.get(args[0])
    ...
    if (!membro) return message.reply('🚨 | O comando está errado, digite .help para saber como usar``')
    if (membro === message.member) return message.reply(`🚨 | Foi mal, mas cê não pode fazer isso.`)
